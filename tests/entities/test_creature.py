...
class TestCreature(unittest.TestCase):
    ...
    def test_get(self):
        self.assertEqual(self.creature.get_slot(creatures.WEAPONS), slots.Weapons())
        self.assertEqual(self.creature.get_slot(creatures.BOOTS), slots.Boots())
        with self.assertRaises(ValueError):
            self.creature.get_slot("Weapon")
    ...
    def test_class_initializer(self):
        ...
        self.assertEqual(creature_A.get_slot(creatures.WEAPONS), 
                        creature_B.get_slot(creatures.WEAPONS))

        self.assertIsNot(creature_A.get_slot(creatures.WEAPONS), 
                        creature_B.get_slot(creatures.WEAPONS))

        self.assertEqual(creature_A.get_slot(creatures.COINS), 
                        creature_B.get_slot(creatures.COINS))
        self.assertIsNot(creature_A.get_slot(creatures.COINS), 
                        creature_B.get_slot(creatures.COINS))
    ...
    def test_find_equippable_slot(self):
        self.assertEqual(self.creature.find_equippable_slot(self.dagger), slots.OneHanded())
    ...
    def test_equip_non_eqippable_item(self):
        ...
        with self.assertRaises(creatures.NonEquippableError):
            self.creature.equip_item(self.copper_coin)

        self.creature.add_item(self.bread)
        with self.assertRaises(creatures.NonEquippableError):
            self.creature.equip_item(self.bread)
    ...
    def test_unequip_non_eqippable_item(self):
        self.creature.add_item(self.copper_coin)
        with self.assertRaises(creatures.NonEquippableError):
            self.creature.unequip_item(self.copper_coin)
    ...
    def test_type_error(self):
        ...
        for case in test_case:
            ...
            self.assertRaises(TypeError, func, case)
            self.assertRaises(TypeError, func, (self.dagger, case))
            func = self.creature.equip_item
            self.assertRaises(TypeError, func, case)
            func = self.creature.unequip_item
            self.assertRaises(TypeError, func, case)
